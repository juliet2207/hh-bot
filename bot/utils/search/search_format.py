...
def format_salary(salary: dict | None, lang: str) -> str:
    ...
    if from_str and to_str:
        return t("search.salary.range", lang).format(
            salary_from=from_str, salary_to=to_str, currency=currency
        )
    if from_str:
        return t("search.salary.from_only", lang).format(
            salary_from=from_str, currency=currency
        )
    if to_str:
        return t("search.salary.to_only", lang).format(
            salary_to=to_str, currency=currency
        )
    return t("search.salary.not_specified", lang)
...
def format_vacancy_details(
    vacancy: dict, position: int, total_found: int, lang: str
) -> str:
    ...
    requirements = vacancy.get("requirements") or vacancy.get("snippet", {}).get(
        "responsibility", ""
    )
...
def format_search_page(
    ...
    response = (
        t("search.results_header", lang).format(total=total_found, query=query) + "\n\n"
    )
    ...
    response += "\n" + t("search.page_label", lang).format(
...
def format_search_response(
    query: str, results: dict, lang: str, max_results: int = 3
) -> str:
    ...
    response = (
        t("search.results_header", lang).format(total=found_count, query=query) + "\n\n"
    )
...
def create_pagination_keyboard(
    ...
    if page > 0:
        buttons.append(
            {"text": "◀️", "callback_data": f"search_page:{query}:{page - 1}"}
        )
    ...
    if total_pages <= 7:
        for p in range(1, total_pages + 1):
            ...
            buttons.append(
                {"text": text, "callback_data": f"search_page:{query}:{p - 1}"}
            )
    else:
        ...
        buttons.append(
            {
                "text": ("• 1 •" if current_page_num == 1 else "1"),
...
def create_vacancy_buttons(
    query: str, page: int, per_page: int, total_count: int
) -> list[dict[str, str]]:
    ...
    return [
        {"text": f"{i + 1}.", "callback_data": f"vacancy_detail:{query}:{i}"}
