...
from bot.utils.keyboards import row


def profile_keyboard(
    lang: str, skills_count: int = 0, skills_preview: str | None = None
        ...
        inline_keyboard=[
            row(
                InlineKeyboardButton(
                    text=t("profile.buttons.edit_name", lang), callback_data="edit_name"
                ),
                InlineKeyboardButton(
                    text=t("profile.buttons.edit_city", lang), callback_data="edit_city"
                ),
            ),
            row(
                InlineKeyboardButton(
